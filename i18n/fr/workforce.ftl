...
deployment-sync = Synchronisation du déploiement
deployment-sync-help = Lancez les pipelines Azure DevOps qui compilent depuis GitHub, publient les images de conteneur, mettent à jour GitOps et laissent Argo CD déployer vers GKE.
deployment-config-missing = La synchronisation du déploiement n'est pas entièrement configurée.
deployment-config-help = Définissez AZURE_DEVOPS_ORG, AZURE_DEVOPS_PROJECT, AZURE_DEVOPS_PAT, AZURE_PIPELINE_API_ID et AZURE_PIPELINE_FRONTEND_ID dans l'environnement du frontal.
deployment-targets = Cibles de déploiement
deployment-target = Cible
deployment-flow = Flux de déploiement
deployment-flow-help = L'application ne fait que mettre en file d'attente des exécutions de pipelines Azure DevOps. Azure Pipelines compile et publie les images, met à jour le dépôt workforce-gitops, puis Argo CD applique le changement dans GKE.
