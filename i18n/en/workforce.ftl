...
deployment-sync-help = Trigger Azure DevOps pipelines that build from GitHub, publish container images, update GitOps, and let Argo CD deploy to GKE.
deployment-config-missing = Deployment sync is not fully configured.
deployment-config-help = Set AZURE_DEVOPS_ORG, AZURE_DEVOPS_PROJECT, AZURE_DEVOPS_PAT, AZURE_PIPELINE_API_ID, and AZURE_PIPELINE_FRONTEND_ID in the frontend environment.
deployment-targets = Deployment targets
deployment-target = Target
deployment-flow = Deployment flow
deployment-flow-help = The app only queues Azure DevOps pipeline runs. Azure Pipelines builds and pushes images, updates the workforce-gitops repository, and Argo CD applies the change to GKE.
